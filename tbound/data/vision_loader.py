...
def _build_stl10_cache(data_path: str) -> None:
    print("  [stl10] Loading into cache...", flush=True)
    ...
    ds_train_val = datasets.STL10(data_path, split="train", download=False,
                                  transform=val_tf)
    ds_test      = datasets.STL10(data_path, split="test",  download=False,
                                  transform=val_tf)
    train_idx, val_idx = _load_val_split("stl10")
    _cache["stl10"] = {
        "ds_train_val": ds_train_val,
        "ds_test":      ds_test,
        "train_idx":    train_idx,
        "val_idx":      val_idx,
        "data_path":    data_path,
    }
    print(f"  [stl10] Cached {len(ds_train_val):,} train + {len(ds_test):,} test.",
          flush=True)
...
def _loaders_stl10(dataset_fraction: float, batch_size: int,
    ...
    ds_train_aug = datasets.STL10(
        c["data_path"], split="train", download=False, transform=train_tf
    )
    ...
    return (
        _make_loader(Subset(ds_train_aug,      chosen),        batch_size,     shuffle=True),
        _make_loader(Subset(c["ds_train_val"], c["val_idx"]),  batch_size * 2, shuffle=False),
        _make_loader(c["ds_test"],                             batch_size * 2, shuffle=False),
        len(chosen),
    )
